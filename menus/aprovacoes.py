    ...
    with engine.connect() as conn:
        for item, tamanho, quantidade, status_item in itens_editados_lista:
            ...
            conn.execute(query, {
                ...
                "efetivo": dados_aprovados['efetivo'], "turno": dados_aprovados['turno'], "cc": dados_aprovados['centro_de_custo'],
    ...
    with engine.connect() as conn:
        for item, tamanho, quantidade in itens_editados_lista: 
            query = text("""
                ...
                (data, cpf, coordenador, colaborador, item, turno, quantidade, tamanho, responsavel, email_coordenador, centro_de_custo)
    ...
    with engine.connect() as conn:
        for item, tamanho, quantidade, status_item in itens_editados_lista:
            query = text("""
                ...
                (data, colaborador, cpf, coordenador, email_coordenador, responsavel, status, turno, centro_de_custo, item, tamanho, quantidade, assinatura)
                VALUES (:data, :colab, :cpf, :coord, :email, :resp, :status, :turno, :cc, :item, :tam, :qtd, :ass)
            """)
            conn.execute(query, {
                "data": dados_aprovados['data'], "colab": dados_aprovados['colaborador'], "cpf": dados_aprovados['cpf'],
                "coord": dados_aprovados['coordenador'], "email": dados_aprovados['email_coordenador'],
        ...
        if df_epi.empty:
            ...
            for index, row in df_epi.iterrows():
                ...
                with st.expander(f"Solicitação #{p_id} - {row['colaborador']} ({row['data']})", expanded=False):
                    ...
                    with col2:
                        n_email = st.text_input("E-mail Coordenador", value=row['email_coordenador'], key=f"epi_email_{p_id}")
                    ...
                    df_itens = pd.DataFrame(json.loads(row['itens_json']), columns=["Item", "Tamanho", "Quantidade", "Status"])
                    df_editado = st.data_editor(df_itens, key=f"epi_editor_{p_id}", use_container_width=True, hide_index=True)

                    if row.get('assinatura'):
                        st.markdown(f"<img src='{row['assinatura']}' width='250' style='border:1px solid #ccc; border-radius:8px;'>", unsafe_allow_html=True)
                    
                    col_aprov, col_recusa = st.columns(2)
                    with col_aprov:
                        if st.button("✅ Aprovar Saída", key=f"epi_aprov_{p_id}", type="primary", use_container_width=True):
                            dados_atualizados = {
                                ...
                                'centro_de_custo': n_cc, 'turno': n_turno, 'efetivo': n_efetivo, 'assinatura': row.get('assinatura')
                            }
                            ...
                            with st.spinner("Aprovando..."):
                                ...
                                for item_nome, tam, qtd, status_item in itens_finais:
                                    atualizar_estoque(item_nome=item_nome, tamanho=tam, status=status_item, tipo="EPI", quantidade_delta=-int(qtd))
                                ...
                                except Exception as e: st.warning(f"Erro e-mail: {e}")
            ...
            for index, row in df_insumo.iterrows():
                ...
                with st.expander(f"Solicitação #{p_id} - {row['colaborador']} ({row['data']})", expanded=False):
                    ...
                    with col1:
                        ...
                        n_cc = st.text_input("Centro de Custo", value=row.get('centro_de_custo', ''), key=f"ins_cc_{p_id}")
                    with col2:
                        n_email = st.text_input("E-mail Coordenador", value=row['email_coordenador'], key=f"ins_email_{p_id}")
                    ...
                    df_editado = st.data_editor(df_itens, key=f"ins_editor_{p_id}", use_container_width=True, hide_index=True)
                    
                    st.write("")
                    col_aprov, col_recusa = st.columns(2)
                    with col_aprov:
                        if st.button("✅ Aprovar Insumo", key=f"ins_aprov_{p_id}", type="primary", use_container_width=True):
                            dados_atualizados = {
                                'data': row['data'], 'colaborador': n_colab, 'cpf': n_cpf, 'coordenador': n_coord,
                                'email_coordenador': n_email, 'responsavel': n_resp, 'turno': n_turno, 'centro_de_custo': n_cc
                            }
                            itens_finais = list(df_editado.itertuples(index=False, name=None))
                            ...
                            with st.spinner("Aprovando..."):
                                ...
                                for item_nome, tam, qtd in itens_finais:
                                    atualizar_estoque(item_nome=item_nome, tamanho=tam, status="NOVO", tipo="INSUMO", quantidade_delta=-int(qtd))
                                
                                try:
                                    enviar_email_saida_insumos(
                                        cpf=n_cpf, coordenador=n_coord, colaborador=n_colab, responsavel=n_resp,
                                        email_coordenador=n_email, turno=n_turno, centro_de_custo=n_cc, itens=itens_finais
                                    )
                                except Exception as e: st.warning(f"Erro e-mail: {e}")
        ...
        if df_emp.empty:
            ...
            for index, row in df_emp.iterrows():
                ...
                with st.expander(f"Solicitação #{p_id} - {row['colaborador']} ({row['data']})", expanded=False):
                    ...
                    with col2:
                        n_email = st.text_input("E-mail Coordenador", value=row['email_coordenador'], key=f"emp_email_{p_id}")
                    ...
                    st.markdown("**Itens Solicitados:**")
                    df_itens = pd.DataFrame(json.loads(row['itens_json']), columns=["Item", "Tamanho", "Quantidade", "Status"])
                    df_editado = st.data_editor(df_itens, key=f"emp_editor_{p_id}", use_container_width=True, hide_index=True)
                    ...
                    col_aprov, col_recusa = st.columns(2)
                    with col_aprov:
                        if st.button("✅ Aprovar Empréstimo", key=f"emp_aprov_{p_id}", type="primary", use_container_width=True):
                            dados_atualizados = {
                                'data': row['data'], 'colaborador': n_colab, 'cpf': n_cpf, 'coordenador': n_coord,
                                'email_coordenador': n_email, 'responsavel': n_resp, 'turno': n_turno, 'centro_de_custo': n_cc,
